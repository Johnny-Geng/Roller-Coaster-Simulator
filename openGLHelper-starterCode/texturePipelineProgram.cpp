...
int TexturePipelinePorgram::Init(const char * shaderBasePath) 
{
...
void TexturePipelinePorgram::SetModelViewMatrix(const float * m) 
{
  // Pass "m" to the pipeline program, as the modelview matrix.
  glUniformMatrix4fv(h_modelViewMatrix, 1, GL_FALSE, m);
}

void TexturePipelinePorgram::SetProjectionMatrix(const float * m) 
{
  // Pass "m" to the pipeline program, as the projection matrix.
  glUniformMatrix4fv(h_projectionMatrix, 1, GL_FALSE, m);
}

int TexturePipelinePorgram::SetShaderVariableHandles() 
{
